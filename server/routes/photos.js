...
router.patch("/batch-claim", verifyToken, async (req, res) => {
    try {
        ...
        const photos = await Photo.find({ _id: { $in: photoIds }, status: "unclaimed" });
        if (photos.length === 0) {
            return res.status(400).json({ message: "No unclaimed photos found with the provided IDs" });
        }
        const updateResult = await Photo.updateMany(
            { _id: { $in: photos.map((p) => p._id) } },
            { $set: { status: "claimed", initials: user.initials, lastUpdated: new Date() } }
        );
        res.json({
...
router.patch("/:id/claim", verifyToken, async (req, res) => {
    try {
        ...
        if (photo.status !== "unclaimed") {
            return res.status(400).json({ message: "Photo is already claimed or processed" });
        }
...
router.patch("/:id/unclaim", verifyToken, async (req, res) => {
    try {
        ...
        if (photo.initials !== user.initials) {
            return res.status(403).json({ message: "You can only unclaim photos you have claimed" });
        }
        if (photo.status === "finished") {
            return res.status(400).json({ message: "Finished photos cannot be unclaimed" });
        }
...
router.patch("/:id/update", verifyToken, async (req, res) => {
    try {
        ...
        if (!photo) return res.status(404).json({ message: "Photo not found" });
        if (photo.status !== "claimed" && photo.status !== "in_progress") {
            return res.status(400).json({ message: "Photo must be claimed to update" });
        }
        if (photo.initials !== user.initials) {
            return res.status(403).json({ message: "You can only update photos you have claimed" });
        }
        if (updateData.municipality !== undefined) {
            if (!updateData.municipality || updateData.municipality === "" || updateData.municipality === "Unknown") {
                return res.status(400).json({ message: "Municipality is required and cannot be empty or 'Unknown'" });
            }
        }
...
router.get("/my-claimed", verifyToken, async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Error getting user's claimed photos" });
    }
});
